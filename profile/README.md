## Welome to the AssemblyAI Solutions GitHub Repo.

Here you can find several resources that will enable you to more effectively utilize the AssemblyAI API.

### Deployment Examples
- [IP-less Webhook Deployments](https://github.com/AssemblyAI-Solutions/assemblyai_webhook_example)
- [Ingest HLS Livestream](https://github.com/AssemblyAI-Solutions/HLS_Stream)
- [Chunk audio to accelerate transcription speed](https://github.com/AssemblyAI-Solutions/FAST)

### Benchmarking
- [Realtime Benchmark](https://github.com/AssemblyAI-Solutions/realtime_benchmark)
- [Audio Intelligence Summarization vs LeMUR Summary](https://aai-summarization-comparisons.streamlit.app)

### Use Case Examples
- [Realtime IVR Agent](https://github.com/AssemblyAI-Solutions/LeMUR-Realtime-Phone-Assistant)
